...
# Extraer columnas para el plot
tiempos = numeric_data[:, 0]
g_parciales = numeric_data[:, 2]

# Calcular G_total y desviación estándar
G_total = np.sum(g_parciales)
std_g = np.std(g_parciales)

# Crear el plot
plt.figure(figsize=(10, 5))
plt.plot(tiempos, g_parciales, label="G_parcial", color='blue')
plt.xlabel("Tiempo (s)")
plt.ylabel("G_parcial")
plt.title(f"G_total: {G_total:.2f} | Desviación estándar: {std_g:.2f}")
plt.grid(True)
plt.tight_layout()
plt.savefig("imgs/tiempo_gparcial.png")
plt.show()
